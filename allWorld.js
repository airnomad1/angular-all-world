(function (angular) {
    ...
    var allWorld = angular.module('allWorld', []);

    allWorld.provider('$world', ['allWorldConfig', function (allWorldConfig) {
        var enable = false;

        this.allWorld = function (arg) {

        };

        this.$get = ['$http', function () {
            return new World(enable);
        }];
    }]);

    function World(enable) {
        this.enable = enable;
    }

    var fn = function () {
        // All world factories
        allWorld.factory('searchWorld', ['$http', function ($http) {
            var fn = {
                continents: function () {
                    var worldContinents = [];
                    for (var continent in $continents) {
                        worldContinents.push(continent);
                    }
                ...
                countries: function () {
                    var worldCountries = [];
                    for (var continent in $continents) {
                        var countries = $continents[continent].split('|');
                        for (var x = 0; x < countries.length; x++) {
                            worldCountries.push({name: countries[x], value: countries[x]});
                        }
                    }
                    return worldCountries.sort(function (a, b) {
                        return
                ...
                states: function () {
                    var worldStates = [];
                    for (var countries in $states) {
                        var states = $states[countries].split('|');
                        for (var x = 0; x < states.length; x++) {
                            worldStates.push({name: states[x], value: states[x]});
                        }
                    }
                ...
                countryStates: function (country) {
                    try {
                        var countryStates = [];
                        var states = $states[country].split('|');
                        for (var x = 0; x < states.length; x++) {
                            countryStates.push({name: states[x], value: states[x]});
                        }
                        return countryStates.sort(function (a, b) {
                            var a = a.name.toUpperCase();
                            var b = b.name.toUpperCase();
                ...
                getGeoCities: function (geonameId) {

                },
                getUsStates: function () {
                    var states = [];
                    for (var key in us_states) {
                        states.push({name: us_states[key], value: key});
                    }
                    return states;
                }
            };
        ...
        // All world directives
        allWorld.directive('worldContinents', [function () {
            return {
                replace: true,
                restrict: 'E',
                require: 'ngModel',
                template: function (element, attrs) {
                    var selectBox = '<select id="worldContinents" name="worldContinents" ng-options="continent for continent in worldContinents"><option disabled value="">Select Continent</option></select>';
                    return selectBox;
                },
                link: function (scope, element, attrs, ctrl) {
                    ...
                    scope.worldContinents = continents;

                    scope.$watch(function () {
                        return ctrl.$modelValue;
                    }, function (value) {
                        if (value) {
                            var countries = $continents[value].split('|');
                            scope.worldCountries = countries;
                        }
                    });

                    // On Change
                    element.bind('change', function (event) {
                        var countries = $continents[ctrl.$viewValue].split('|');
        ...
        allWorld.directive('worldCountries', ['$http', 'searchWorld', function ($http, searchWorld) {
            return {
                replace: true,
                restrict: 'E',
                require: 'ngModel',
                template: function (element, attrs) {
                    var selectBox = '<select id="worldCountries" name="worldCountries" ng-selected="' + attrs.ngModel + '" ng-options="country for country in worldCountries"><option disabled value="">Select Country</option></select>';
                    return selectBox;
                },
                link: function (scope, element, attrs, ctrl) {
                    scope.$watch(function () {
                        ...
                        if (value) {
                            var states = $states[value].split('|');
                            scope.worldStates = states;
                        }
                    });

                    // On Change
                    element.bind('change', function (event) {
                        if ($states[ctrl.$viewValue]) {
                            var states = $states[ctrl.$viewValue].split('|');
                            scope.$apply(function () {
                                scope.worldStates = states;
                            });
                        }
        ...
        allWorld.directive('worldStates', ['$http', function ($http) {
            return {
                replace: true,
                restrict: 'E',
                require: 'ngModel',
                template: function (element, attrs) {
                    var selectBox = '<select id="worldStates" name="worldStates" ng-options="state for state in worldStates"><option disabled value="">Select State</option></select>';
                    return selectBox;
                },
                link: function (scope, element, attrs, ctrl) {
        ...
        allWorld.directive('worldCounties', [function () {
            return {
                replace: true,
                restrict: 'E',
                template: function (element, attrs) {
                    var selectBox = '<input id="worldCounties" name="worldCounties" />';
                    return selectBox;
                },
        ...
        allWorld.directive('geoAutoComplete', [function () {
            return {
                replace: true,
                restrict: 'A',
                scope: {
                ...
                link: function ($scope, element, attrs) {
                    ...
                    google.maps.event.addListener(autocomplete, 'place_changed', function () {
                        ...
                        var city = null, state = null, country = null, zipcode = null, address = null;
                        address = element[0].value;
                        ...
                        for (var i = 0; i < place.address_components.length; i++) {
                            ...
                            switch (place.address_components[i].types[0]) {
                                ...
                                case 'administrative_area_level_1':
                                    state = ((state != null) ? state : '') + place.address_components[i].long_name;
                                    break;
                                case 'locality':
                                    city = place.address_components[i].long_name;
                                    break;
                                case 'postal_code':
                                    zipcode = ((zipcode != null) ? zipcode : '') + place.address_components[i].short_name;
                                    break;
                            }
                        }

                        if (document.getElementById($scope.city)) {
                            document.getElementById($scope.city).value = city;
                        }

                        if (document.getElementById($scope.state)) {
                            document.getElementById($scope.state).value = state;
                        }

                        if (document.getElementById($scope.country)) {
                            document.getElementById($scope.country).value = country;
                        }

                        if (document.getElementById($scope.zipcode)) {
                            document.getElementById($scope.zipcode).value = zipcode;
                        }

                        $scope.$apply(function () {
                            $scope.update.address = address;
                            $scope.update.city = city;
                            $scope.update.state = state;
                            $scope.update.country = country;
    ...
    allWorld.constant('allWorldConfig', {
        autoLoadGoogleMap: false
    });
...
var $continents = Object();

$continents['Africa'] = 'Algeria|Angola|Benin|Botswana|Burkina Faso|Burundi|Cameroon|Cape Verde|Central African Republic|Chad|Comoros|Congo, Dem.|Congo, Rep.|Djibouti|Egypt|Equatorial Guinea|Eritrea|Ethiopia|Gabon|Gambia|Ghana|Guinea|Guinea-Bissau|Kenya|Lesotho|Liberia|Libya|Madagascar|Malawi|Mali|Mauritania|Mauritius|Morocco|Mozambique|Namibia|Niger|Nigeria|Rwanda|Sao Tome/Principe|Senegal|Seychelles|Sierra Leone|Somalia|South Africa|Sudan|Swaziland|Tanzania|Togo|Tunisia|Uganda|Zambia|Zimbabwe';
$continents['Antarctica'] = 'Amundsen-Scott';
$continents['Asia'] = 'Bangladesh|Bhutan|Brunei|Burma (Myanmar)|Cambodia|China|East Timor|India|Indonesia|Japan|Kazakhstan|Korea (north)|Korea (south)|Laos|Malaysia|Maldives|Mongolia|Nepal|Philippines|Russian Federation|Singapore|Sri Lanka|Taiwan|Thailand|Vietnam';
$continents['Australia/Oceania'] = 'Australia|Fiji|Kiribati|Micronesia|Nauru|New Zealand|Palau|Papua New Guinea|Samoa|Tonga|Tuvalu|Vanuatu';
$continents['Caribbean'] = 'Anguilla|Antigua/Barbuda|Aruba|Bahamas|Barbados|Cozumel|Cuba|Dominica|Dominican Republic|Grenada|Guadeloupe|Haiti|Jamaica|Martinique|Montserrat|Netherlands Antilles|Puerto Rico|St. Barts|St. Kitts/Nevis|St. Lucia|St. Martin/Sint Maarten|St Vincent/Grenadines|San Andres|Trinidad/Tobago|Turks/Caicos';
$continents['Central America'] = 'Belize|Costa Rica|El Salvador|Guatemala|Honduras|Nicaragua|Panama';
$continents['Europe'] = 'Albania|Andorra|Austria|Belarus|Belgium|Bosnia-Herzegovina|Bulgaria|Croatia|Czech Republic|Denmark|Estonia|Finland|France|Georgia|Germany|Greece|Hungary|Iceland|Ireland|Italy|Latvia|Liechtenstein|Lithuania|Luxembourg|Macedonia|Malta|Moldova|Monaco|Netherlands|Norway|Poland|Portugal|Romania|San Marino|Serbia/Montenegro (Yugoslavia)|Slovakia|Slovenia|Spain|Sweden|Switzerland|Ukraine|United Kingdom|Vatican City';
$continents['Islands'] = 'Arctic Ocean|Atlantic Ocean (North)|Atlantic Ocean (South)|Assorted|Caribbean Sea|Greek Isles|Indian Ocean|Mediterranean Sea|Oceania|Pacific Ocean (North)|Pacific Ocean (South)';
$continents['Middle East'] = 'Afghanistan|Armenia|Azerbaijan|Bahrain|Cyprus|Iran|Iraq|Israel|Jordan|Kuwait|Kyrgyzstan|Lebanon|Oman|Pakistan|Qatar|Saudi Arabia|Syria|Tajikistan|Turkey|Turkmenistan|United Arab Emirates|Uzbekistan|Yemen';
$continents['North America'] = 'Bermuda|Canada|Greenland|Mexico|United States';
$continents['South America'] = 'Argentina|Bolivia|Brazil|Chile|Colombia|Ecuador|Guyana|Paraguay|Peru|Suriname|Uruguay|Venezuela';
...
$states['Singapore'] = '';
$states['Sri Lanka'] = 'Central|North Central|Northern|Eastern|North Western|Sabaragamuwa|Southern|Uva|Western';
